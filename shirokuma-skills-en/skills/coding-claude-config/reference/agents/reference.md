...
Agents run as independent subprocesses — **main context rules are NOT automatically propagated** to agents.

### Propagation Mechanisms

| Mechanism | Method | Use Case |
|-----------|--------|----------|
| `skills:` frontmatter | Inject skills (skill's reference also becomes available) | Delegate specific workflows to agent |
| Initialization Read | Instruct in AGENT.md: "First, Read X with Read tool" | When specific rule file content is needed |
| Inline in prompt | Include rule summary in system prompt | Short rules, low change frequency |
| `context: fork` (Skill) | Skill tool with `context: fork` forks main context to subagent | When subagent needs full main context (rules + conversation) |

> **Note:** When a skill uses `context: fork`, the subagent inherits the main context including all loaded rules. Explicit Read instructions in AGENT.md are unnecessary for forked agents. The mechanisms above apply only to agents launched via Agent tool (independent subprocess).

### Anti-patterns

| Pattern | Problem |
|---------|---------|
| AGENT.md says "follow branch-workflow rule" | Rules don't propagate to subagent — ignored |
| Write rule file path and say "reference" | Not auto-expanded — explicit Read tool instruction needed |

### Recommended Patterns

When instructing Read in the initialization section:

```markdown
## Initialization

1. Read the following files with the Read tool:
   - `.claude/rules/shirokuma/github/branch-workflow.md`
2. Follow the loaded rules during work
```

When injecting via `skills:`:
