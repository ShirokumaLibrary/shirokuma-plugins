...
#### 6d. 修正内容コメントの投稿とレビュー結果の本文反映

セルフレビューループ完了後（PASS またはループ停止）、自動修正が行われた場合は修正内容を集約したコメントを投稿し、その後 Issue 本文の更新を行う。

##### 修正内容コメントの投稿

| レビュー結果 | 修正内容コメント |
|-------------|----------------|
| PASS（問題なし） | 不要。レビューコメント自体がコメントファーストの「コメント」に該当する |
| PASS + out-of-scope あり | 不要。フォローアップ Issue の作成は Step 6c で処理済み |
| FAIL → 自動修正 → PASS | **必要**。全イテレーションの修正を集約し、コミットハッシュを参照する |
...
- **本文への統合**: レビュー指摘に基づき、Issue 本文の該当セクション（タスクリスト、成果物等）を更新する。具体的な手順パターンは `item-maintenance.md` の「レビュー結果からの本文更新」セクションを参照。

**条件付き実行**: レビューが PASS で指摘がない場合、本文更新は不要のためこのステップをスキップ。

##### セルフレビュー完了報告

6d の処理完了後、ユーザーにセルフレビュー結果を報告する:
