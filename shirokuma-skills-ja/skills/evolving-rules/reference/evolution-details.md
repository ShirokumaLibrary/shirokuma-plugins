...
Issue が 0 件の場合、`/tmp/shirokuma-docs/evolution.md` に以下を書き出し、`items add issue --file` で作成する:
...
主要スキル（`implement-flow`, `prepare-flow`, `creating-item`, `design-flow`, `review-flow`）の完了時に、以下の手順でセッション中に発生した Evolution シグナルを自動記録する。各スキルはこのセクションを参照して自動記録を実行する。
...
コード変更を伴うスキル完了時に、プロジェクトの客観的状態を確認する。`prepare-flow` / `creating-item` / `design-flow` はコード変更を伴わないため対象外。
...
2. シグナルをコメントとして投稿（複数シグナルは 1 コメントに集約）:
   ```bash
   cat > /tmp/shirokuma-docs/{number}-signal.md <<'EOF'
   **種別:** {種別}
   **対象:** {ルール名 or スキル名}
...
- シグナル未検出時は CLI コマンド実行を最小化（`items list` 1 回のみ）
...
# 2. シグナルをコメントとして投稿
cat > /tmp/shirokuma-docs/{issue-number}-signal.md <<'EOF'
