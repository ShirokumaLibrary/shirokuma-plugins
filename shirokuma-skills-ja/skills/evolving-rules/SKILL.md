...
- **スコープ外:** ルール・スキルファイルの直接変更（提案は Issue として記録し、実装は `working-on-issue` ワークフローに委任）
...
```bash
shirokuma-docs issues comment {evolution-number} --body "提案 Issue を作成しました: #{created-issue-number}"
```
...
| 対象 | カテゴリ | アクション |
|------|---------|----------|
| {name} | {category} | {Issue 作成 (#NNN) / スキップ} |
```
...
- `evolving-rules` はルール・スキルの変更を直接行わない。提案は Issue として記録し、実装は `working-on-issue` ワークフローに委任する
