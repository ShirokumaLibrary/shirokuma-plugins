...
description: "要件定義フェーズ全体を統括するオーケストレーター。既存 ADR・Discussion の検索・整合確認、ADR 作成（write-adr に委任）、仕様 Discussion 作成を一貫して実行する。トリガー: 「要件定義」「要件整理」「ADR作成して」「仕様作成して」「仕様を整理したい」「技術選定を記録したい」「アーキテクチャ決定を残したい」。Issue/Discussion を今すぐ登録したいだけなら /create-item-flow を使用してください。"
...
| 2 | 関連 Discussion 検索 | 既存 Discussion を検索中 | Bash: `shirokuma-docs items adr list` + `items discussions search` |
...
| 仕様関連キーワード（「仕様」「要件定義」「要件整理」「スペック」「機能要件」「非機能要件」） | 仕様 Discussion 作成（Bash: `shirokuma-docs items add discussion`） |
...
Agent(
  ...
  subagent_type: "requirements-worker",
  prompt: "仕様 Discussion を作成してください。\n`shirokuma-docs items add discussion` を Bash で直接実行してください（Ideas カテゴリ、タイトルに [Spec] プレフィックス）。\n\nコンテキスト:\n{ユーザー発話の内容}\n\n関連 Discussion（参考）:\n{ステップ 2 の検索結果サマリー}"
)
```
...
Agent(
  ...
  subagent_type: "requirements-worker",
  prompt: "以下の 2 つを順次実行してください:\n1. write-adr を使って ADR を作成する\n2. `shirokuma-docs items add discussion` を Bash で直接実行して仕様 Discussion を作成する（Ideas カテゴリ、タイトルに [Spec] プレフィックス）\n\nコンテキスト:\n{ユーザー発話の内容}\n\n関連 Discussion（参考）:\n{ステップ 2 の検索結果サマリー}"
)
```
