...
> **チェーン自律進行**: 計画レビュー fork（ステップ 5）が結果を返した後、即座にステップ 6-7（ステータス更新、ユーザーへの返却）に進んでください。レビュー fork 結果で停止するとユーザーが手動で継続を促す必要が生じ、計画ワークフローが中断します。YAML フロントマターの `action` フィールドをパースし、ユーザー入力を待たずに進行してください。
...
レビュー前に Issue 本文へ計画セクションを書き込む。これにより `reviewing-on-issue` が `shirokuma-docs issues show {number}` で計画内容を取得してレビューできる。
...
### ステップ 5: 計画レビュー（fork 委任）

計画策定と同じエージェントがレビューしても盲点に気づけない。`reviewing-on-issue` の plan ロールに fork 委任し、まっさらなコンテキストでレビューを実行する。ステップ 4 で Issue 本文に計画が書き込まれているため、reviewer は `shirokuma-docs issues show {number}` で計画内容を取得できる。

#### スキル利用可能チェック（フォールバック）

fork 起動前に `reviewing-on-issue` スキルがスキルリストに存在するか確認する。
...
Skill ツールで `reviewing-on-issue` を plan ロールで起動する。`reviewing-on-issue` が自身で `shirokuma-docs issues show {number}` を実行して Issue 本文（ステップ 4 で書き込み済みの計画を含む）を取得するため、prompt への埋め込みは不要。
...
2. 続けて「5a: コメントで計画の判断根拠を投稿」を実行する。
...
3. **[計画]** の問題 → 計画セクションを修正し、Issue 本文の `## 計画` セクションを修正版で更新する
...
```
計画策定 → 本文書き込み → Skill(reviewing-on-issue plan) → NEEDS_REVISION → 計画修正 + 本文更新 → 再レビュー → PASS → 対応コメント → ステップ 5a
                                                                                               ↓ (2回失敗)
                                                                                         ユーザーに報告
```

### ステップ 6: ステータス更新（計画レビュー完了後）
...
| Skill (reviewing-on-issue) | ステップ 5: まっさらコンテキストでの計画レビュー（fork 委任） |
...
- **チェーン自律進行**: レビュー fork（ステップ 5）が結果を返した後、停止するとユーザーが手動で継続を促す必要が生じる。YAML フロントマターの `action` フィールドに基づき即座にステップ 6-7 に進む
