...
> **注意:** `creating-item` からのチェーンは `plan-issue` に直接委任する（`working-on-issue` ではない）。新規作成された Issue には計画が存在しないため、`working-on-issue` のルーティングを経由するのは冗長。
