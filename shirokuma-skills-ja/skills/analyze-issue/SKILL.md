...
1. `shirokuma-docs issue context {number}` で Issue 本文を取得し、`.shirokuma/github/{org}/{repo}/issues/{number}/body.md` を Read ツールで読み込む
...
1. `shirokuma-docs issue context {number}` で親 Issue を取得し、`.shirokuma/github/{org}/{repo}/issues/{number}/body.md` を Read ツールで読み込む
2. `subIssuesSummary` からタイトルが「計画:」で始まる計画 Issue を特定する
3. `shirokuma-docs issue context {plan-issue-number}` で計画 Issue の本文を取得し、`.shirokuma/github/{org}/{repo}/issues/{plan-issue-number}/body.md` を Read ツールで読み込む
