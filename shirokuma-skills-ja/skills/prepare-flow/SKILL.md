...
| 6 | デザインフェーズ要否を判定しステータスを更新する | デザインフェーズ要否を判定しステータスを更新中 | マネージャー直接: `shirokuma-docs items transition / update` |
...
| 設計フェーズ必要 | → In Progress | `design-flow` の実行を案内 |

```bash
# 設計フェーズ不要の場合: ステータスを Review に遷移
shirokuma-docs items transition {number} --to Review

# 設計フェーズ必要の場合: ステータスを In Progress に維持
shirokuma-docs items transition {number} --to "In Progress"
```
...
**必須フィールド**（全レベル共通）:
- **ステータス:** 現在のステータス（Review または In Progress）
