...
|---|---------|------------|--------|
| 1 | Issue を取得しステータスを更新する | Issue を取得しステータスを更新中 | マネージャー直接: `shirokuma-docs issue context / transition` |
...
| 4a | [条件付き] エピック計画のサブ Issue を作成する | サブ Issue を作成中 | マネージャー直接: `shirokuma-docs issue add/parent/push` |
| 6 | ステータスを更新し計画サマリーをユーザーに返す | ステータスを更新し計画サマリーをユーザーに返却中 | マネージャー直接: `shirokuma-docs status transition / update` |
...
| 現在ステータス | アクション |
|--------------|----------|
| Backlog | In Progress に遷移して計画開始を記録 |
| In Progress | スキップ（計画継続） |
| Review | **AskUserQuestion で再計画開始を確認** → 「はい」なら In Progress に戻す（計画レビュー完了状態の上書きを防ぐため明示確認が必要） |

```bash
# Backlog または Review (再計画承諾後) の場合
shirokuma-docs status transition {number} --to "In progress"
# 担当者を追加
shirokuma-docs issue update {number} --assign "@me"
```

> **Review 状態での再計画**: Review は「計画レビュー完了・実装承認待ち」を意味する明示シグナルであり、再計画はユーザー意図の確認が必要。`AskUserQuestion` で「Issue は Review (実装承認待ち) です。再計画を開始しますか？」を提示し、「はい」を選んだ場合のみ In Progress に戻す。「いいえ」の場合は計画作業をキャンセルし呼び出し元に戻る。
...
- **リセット（全サブ Issue をキャンセルして再計画）**: 以下の手順で実行:
  1. 計画 Issue 以外の全サブ Issue を `shirokuma-docs issue cancel {sub-numbers}` で **Done(NOT_PLANNED)** に変更（内部で `state_reason: not_planned` の Close + Status: Done として記録、#2204。子 Issue がある場合は `issue cancel` 内部で自動 unparent される）
  2. 親 Issue を `shirokuma-docs status transition {parent-number} --to "Backlog" --force` で Backlog に強制遷移（`integrity --fix` の親自動同期は `state_reason: not_planned` を判別せず親を Done に遷移させるため、`--force` 付き手動リセットが正規パス）
  3. ステップ 1b に戻り In Progress に再遷移してから ステップ 3（plan-issue 委任）へ進む

  > **代替手段**: サブ Issue 自体に PR や孫 Issue が紐付いている場合は、サブ単位で `shirokuma-docs issue rollback {sub-number} --action cancel` を使うほうが安全（関連 PR クローズ + ブランチ削除 + 孫 unparent + Done 化を一括実行）。リセット手順 1 の代わりに rollback を使い、その後手順 2 (親の Backlog 戻し) に進む。
...
Agent ツールで `review-worker` を plan ロールで起動する。`analyze-issue` が自身で `shirokuma-docs issue context {number}` を実行して Issue 本文を取得する。
...
2. 全サブ Issue 作成後、エピックの `### サブ Issue 構成` テーブルのプレースホルダー（`#{sub1}` 等）を実際の Issue 番号で更新し、`issue update {parent-number} --body /tmp/shirokuma-docs/{parent-number}-body.md` で Issue 本文を同期する。
