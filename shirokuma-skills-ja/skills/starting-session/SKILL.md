...
| Preparing | `preparing-on-issue` | 計画策定中 |
| Designing | `designing-on-issue` | 設計中 |
| Spec Review | `working-on-issue` | 暗黙承認で実装開始 |
| Ready | `working-on-issue` | 着手可能、実装開始 |
...
- 優先度順に表示
- アイテム選択後はステータスベースルーティングに従い `working-on-issue` または `plan-issue` に委任（ステータス更新・ブランチ作成は委任先スキルが担当）
