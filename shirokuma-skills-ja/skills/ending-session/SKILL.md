...
|---|---------|------------|--------|
| 1 | セッションサマリーを収集する | セッションサマリーを収集中 | ステップ 1 |
| 2 | 変更情報を取得する | 変更情報を取得中 | ステップ 2 |
| 3 | プッシュ・PR を作成する | プッシュ・PR を作成中 | ステップ 3（フィーチャーブランチの場合） |
| 4 | セッションサマリー/引き継ぎを作成する | セッションサマリー/引き継ぎを作成中 | ステップ 3.5-3.6 |
| 5 | 保存+ステータス更新する | 保存+ステータス更新中 | ステップ 4 |
| 6 | サマリーを表示する | サマリーを表示中 | ステップ 5 |

Dependencies: step 2 blockedBy 1, step 3 blockedBy 2, step 4 blockedBy 3, step 5 blockedBy 4, step 6 blockedBy 5.

TaskUpdate で各ステップの実行開始時に `in_progress`、完了時に `completed` に更新する。ステップ 3 は条件付き — フィーチャーブランチでない場合は即座に `completed` にしてステップ 4 に進む。
...
`commit-issue` スキルのワークフローに従い、特定ファイルをステージ、Conventional Commits メッセージを作成、コミット。
...
セッションサマリーは**セッションレベルのコンテキスト**に焦点を当てる — 横断的な決定、ブロッカー、次のステップ。`implement-flow` が既に技術的な作業サマリーを Issue に投稿済みの場合、その内容は繰り返さない。
