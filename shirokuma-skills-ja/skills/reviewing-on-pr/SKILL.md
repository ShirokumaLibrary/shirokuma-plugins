...
| スキル | 責務 |
|--------|------|
| `reviewing-on-issue` | コードレビュー実行エンジン。`review-worker` 経由で起動 |
...
3. レビュー実行後、未解決スレッドを確認する:
   ```bash
   shirokuma-docs pr comments {PR#}
   ```
   - 未解決スレッドあり → ステップ 2b（レビュー結果確認）へ
   - 未解決スレッドなし → 完了レポートを表示して終了（ステップ 6 参照）
...
ステップ 2a の `review-worker` 完了後、未解決スレッドがある場合にレビュー結果をユーザーに提示し対応方針を確認する。
