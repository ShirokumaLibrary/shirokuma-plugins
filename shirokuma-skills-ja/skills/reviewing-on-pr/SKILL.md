...
PR 番号を受け取り、コードレビュー実行（`review-issue` Skill 経由）および未解決レビュースレッドの対応（分類・修正・コミット・返信・解決）を自動チェーンで処理する。

## 責務境界

| スキル | 責務 |
|--------|------|
| `review-issue` | コードレビュー実行エンジン。Skill ツール経由で起動 |
...
レビューがまだ提出されていない場合、`review-issue` を Skill ツールで呼び出してコードレビューを実行する。

1. `review-issue` を Skill ツールで起動し、PR の diff に対するコードレビューを実行:
   ```text
   Skill(
     skill: "review-issue",
     args: "code PR #{PR#}"
   ...
   ```
2. `review-issue` がレビュー結果を PR コメントとして投稿する
3. `review-issue` の出力フロントマターから `comment_id` を抽出する（`review-issue` が issue comment を投稿した場合に存在）
4. 未解決スレッドを確認する:
   ```bash
   shirokuma-docs pr comments {PR#}
   ```
   - 未解決スレッドあり（`unresolved_threads > 0`）→ ステップ 2b（レビュー結果確認）へ
   - 未解決スレッドなし かつ `comment_id` あり → ステップ 2b（レビュー結果確認）へ。`review-issue` が改善提案を issue comment として投稿した場合に該当
...
ステップ 2a の `review-issue` 完了後、未解決スレッドまたは `comment_id`（review-issue が投稿した issue comment）がある場合にレビュー結果をユーザーに提示し対応方針を確認する。`review-issue` は指摘を review thread として投稿する場合と issue comment として投稿する場合があり、いずれの形式でも UCP を発動させる。
...
1. **修正**: `code-issue` に修正対象スレッドの情報（ファイルパス、指摘内容）をまとめて渡し、一括修正を委任:
   ...
   ```
   `code-issue` 完了後、`working-on-issue/reference/worker-completion-pattern.md` の統一パターンに従い出力をパースする。
...
7. **レビュー実行は `review-issue` 経由** — ステップ 2a では `review-issue` を Skill ツールで呼び出し、直接レビューを書かない
...
| 状況 | アクション |
|------|----------|
| `review_count: 0` | レビュー実行モード（ステップ 2a）で `review-issue` を Skill ツールで呼び出しコードレビューを実行 |
...
| 未解決スレッドなしだが `comment_id` あり | `review-issue` が改善提案を issue comment で投稿したケース。フロントマターの `comment_id` で識別。ステップ 2b の UCP を発動する |
...
| ツール | タイミング |
|--------|-----------|
| Skill | `review-issue` によるコードレビュー実行（ステップ 2a）、`code-issue` によるコード修正（ステップ 5） |
| Agent | `commit-worker` によるコミット・プッシュ（ステップ 5） |
