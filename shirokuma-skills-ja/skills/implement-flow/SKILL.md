...
| 5 | 作業サマリーを投稿する | 作業サマリーを投稿中 | マネージャー直接: `items add comment` |
| 6 | Status を Review に更新する | Status を Review に更新中 | マネージャー直接: `items transition {number} --to Review` |
...
2. `parent` が設定されている場合 → 親 Issue 番号で `items context` を実行し、以降のフローは親 Issue 番号で実行（計画 Issue の番号は計画コンテキスト参照にのみ使用）
3. `parent` が未設定の場合 → `items context {number}` で再取得して `parent` を確認
4. それでも `parent` が不明な場合 → エラーメッセージを表示して停止:
   「計画 Issue #{number} の親 Issue が特定できません。親 Issue 番号を直接指定してください。」

**Issue 番号あり**: `shirokuma-docs items context {number}` で取得し、JSON 出力から title/body/labels/status/priority/size を抽出。

#### サブ Issue 検出

`.shirokuma/github/{org}/{repo}/issues/{number}/body.md` の frontmatter に `parentIssue` フィールドがある場合、サブ Issue モードで動作する:

1. 親 Issue の計画 Issue（子 Issue のうちタイトルが「計画:」または「Plan:」で始まるもの）を特定し、`items context {plan-issue-number}` で取得して全体コンテキストを把握する
...
| `coding-worker` が `changes_made: false` で完了 | コミット・PR・finalize-changes をスキップ、変更なし用作業サマリーを投稿、AskUserQuestion でステータスを確認（Cancelled 推奨 via `items cancel` / On Hold / Backlog）。[reference/chain-end-steps.md](reference/chain-end-steps.md)「変更なしパス」参照 |
...
2. **サブ Issue の一括作成**（計画 Issue 以外の子 Issue が存在しない場合のみ）: `prepare-flow` で既にサブ Issue が作成済みの場合はこのステップをスキップする。計画 Issue の `### サブ Issue 構成` テーブルを解析し、各行についてサブ Issue を CLI で作成:
   ...
   作成後、計画 Issue の `### サブ Issue 構成` テーブルのプレースホルダー（`#{sub1}` 等）を実際の Issue 番号で更新し、`items update {plan-issue-number} --body /tmp/shirokuma-docs/{plan-issue-number}-body.md` で計画 Issue 本文を同期する。
