...
| 6 | Status を Review に更新する（冪等同期）| Status を Review に更新中 | マネージャー直接: `status transition {number} --to Review` |

Dependencies: step 2 blockedBy 1, step 3 blockedBy 2, step 4 blockedBy 3, step 5 blockedBy 4, step 6 blockedBy 5.

> **ステップ 6 の位置付け（冪等同期）**: ステップ 3 で起動された `pr create` が、PR 本文の `Closes #N` を解析してリンク Issue を **In progress → Review に自動遷移する**（#2240）。ステップ 6 は通常この自動遷移の後で実行されるため、既に Review であれば Status 更新は冪等に成功する。本ステップは `pr create` の自動遷移が何らかの理由（PR 本文に Closes が無い、Issue が想定と異なる Status だった等）で発火しなかった場合のフォールバック同期として残している。
...
| 2 | Discussion に調査結果を保存する | Discussion を作成中 | `shirokuma-docs discussion add` |
...
**Issue 番号あり**: `shirokuma-docs issue context {number}` で取得し、JSON 出力から title/body/labels/status/priority/size を抽出。
...
Issue が In Progress でなければ: `shirokuma-docs status transition {number} --to "In progress"` を実行

**Review からの遷移（明示承認モデル）**: Review から `/implement-flow` を呼び出した行為自体が計画の承認を意味する。この時点で計画 Issue に対して `shirokuma-docs status approve {plan-number}` を実行し、明示的に Done(Open) に遷移させる。その後、対象 Issue を In Progress に遷移。

> **再実行耐性**: `status approve` は計画 Issue が `Review` 以外（既に Done、In Progress 等）の場合 exit 1 で失敗する。再実行や手動遷移後のケースでは JSON 出力の `result: "error"` をスキップして次ステップへ進む（CLI エラーを致命とせず続行）。
...
| `finalize-changes` | Skill ツール | `/simplify` + `reviewing-security` + 改善コミットの後処理チェーン。**Agent ツールで起動しない** |
...
`implement-flow` のチェーン末尾は **Status を Review に更新** までで、**マージはチェーンに含まない**。マージはユーザーが別途 `shirokuma-docs pr merge` で実行する（または GitHub UI から）。

`pr merge` (`src/commands/pr/merge.ts`) は以下の自動アクションを行う:

| 状況 | 自動アクション |
|------|--------------|
| マージ成功 | リンク Issue を Done 化、PR 自身を Done 化、syncParentStatus で親同期 |
| ベースブランチがデフォルトブランチでない（integration ブランチ） | リンク Issue を REST API で明示的に close |
| マージで全実作業サブ Issue が完了 + 全計画完了 | Integration → develop の PR を自動作成し Review に追加、`/review-flow` を提案 |
| マージで全実作業サブ Issue が完了していない（残 Backlog 計画あり） | 次の Backlog 計画を `/implement-flow #{N}` で提案 |

`implement-flow` チェーンを呼ぶユーザーは、マージ後の挙動が CLI 側で自動化されており、追加のオーケストレーター呼び出しは不要であることを覚えておく。マージ後は `pr merge` の `next_action` の指示に従って次のステップに進む。
...
| `coding-worker` が `changes_made: false` で完了 | コミット・PR・finalize-changes をスキップ、変更なし用作業サマリーを投稿、AskUserQuestion でステータスを確認（キャンセル扱い via `issue cancel`（内部で Done + state_reason: not_planned に変換、#2204）/ Blocked / Backlog）。[reference/chain-end-steps.md](reference/chain-end-steps.md)「変更なしパス」参照 |
| Issue が revert された（PR revert 後） | `shirokuma-docs issue rollback {plan-issue#} --action revert` で revert ブランチ作成 + revert PR 作成 + 計画 Issue を Backlog に戻す処理を一括実行する。実行後は新ブランチで再実装。[reference/chain-recovery.md](reference/chain-recovery.md) 参照 |
...
2. **サブ Issue の一括作成**（計画 Issue 以外の子 Issue が存在しない場合のみ）: `prepare-flow` で既にサブ Issue が作成済みの場合はこのステップをスキップする。計画 Issue の `### サブ Issue 構成` テーブルを解析し、各行についてサブ Issue を CLI で作成:
   ...
   作成後、`shirokuma-docs issue parent {sub-number} {epic-number}` でサブ Issue の親を設定する。
   作成後、計画 Issue の `### サブ Issue 構成` テーブルのプレースホルダー（`#{sub1}` 等）を実際の Issue 番号で更新し、`issue update {plan-issue-number} --body /tmp/shirokuma-docs/{plan-issue-number}-body.md` で計画 Issue 本文を同期する。
...
このフローでのサブ Issue 作成は `shirokuma-docs issue add` を直接使用する（`create-item-flow` ではない）。計画でサブ Issue の詳細が確定済みのため、`create-item-flow` の推論ロジックは不要。
...
| Bash | Git 操作、`shirokuma-docs issue` コマンド |
