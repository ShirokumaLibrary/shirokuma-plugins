...
| 状況 | 推奨操作 |
|------|---------|
| サブ Issue 作業完了（レビュー待ち） | `shirokuma-docs status update-batch --review {sub-issue-number}` でサブ Issue のみ更新 |
| 全サブ Issue 完了・最終 PR マージ後 | `shirokuma-docs status update-batch --done {epic-number}` でエピックを Done に |
...
3. **サブ Issue 作成**: `### サブ Issue 構成` テーブルを解析 → 各行について `shirokuma-docs issue add` を frontmatter に `parent: {epic-number}` を含めて実行
...
エピック開始時のサブ Issue 作成は `shirokuma-docs issue add` を直接使用する（`create-item-flow` ではない）。計画でサブ Issue のメタデータが確定済みのため、`create-item-flow` の推論ロジックは不要。
