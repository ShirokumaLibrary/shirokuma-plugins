...
複数ロールが要求された場合、このスキルはロールごとに繰り返し実行される。

### 動作の違い

| 項目 | 通常（単一ロール） | マルチロール |
|------|-------------------|------------|
| ロール選択 | ユーザーリクエストから判定 | 呼び出し元が指定したロールを使用 |
...
- **呼び出し元のコメントファースト遵守**: このスキルはコメント投稿のみを行い本文更新は行わない。呼び出し元スキル（`open-pr-issue`, `working-on-issue`）がレビュー結果に基づいて Issue/PR 本文を更新する場合は、`item-maintenance.md` のコメントファースト原則に従うこと

## 計画レビューモード

`preparing-on-issue` から plan ロールで Skill ツール経由で起動された場合、計画レビュー結果を Issue コメントに投稿し、構造化データを返す。
...
design ロールで Skill ツール経由で起動された場合、設計レビュー結果を Issue コメントに投稿し、構造化データを返す。
...
research ロールで Skill ツール経由で起動された場合、リサーチレビュー結果を Issue コメントに投稿し、構造化データを返す。
...
スタンドアロンまたは Skill ツール経由で起動され、計画レビューでも設計レビューでもない場合は、レポートを GitHub に保存し構造化データを返す。
