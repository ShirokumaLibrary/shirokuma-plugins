---
name: review-issue
description: 専門ロール別の包括的レビューワークフローを提供し、コード品質・セキュリティ・テストパターン・ドキュメント品質・計画品質・要件品質・設計品質・リサーチ品質をチェックします。トリガー: 「レビューして」「review」「セキュリティチェック」「security audit」「テストレビュー」「ドキュメントレビュー」「計画レビュー」「要件レビュー」「要件確認」「設計レビュー」「リサーチレビュー」「コードレビュー」「設定レビュー」「config review」。
...
**対象外ロール:** plan / requirements / design / research はマルチロール自動判定の対象外（Issue 分析ロールであり、コードレビューロールとの混在は自然でない）。
...
| requirements | スキップ（対象が Issue 本文であり、コード/ドキュメントファイルではないため） |
