...
| `code-issue` + `commit-issue` + `open-pr-issue`（並列バッチ） | `parallel-coding-worker` |
...
> **重要 — Agent ツール復帰後のチェーン継続**: カスタムサブエージェント（例: `pr-worker`, `commit-worker`）が完了して Agent ツールが復帰した時点で、**TaskList の残り `pending` ステップを確認する**。pending ステップが残っている場合（作業サマリー、ステータス更新、Evolution）、**同じレスポンス内で即座に次の pending ステップを実行すること**。停止・サマリー表示・ユーザーへの確認は禁止。Agent ツールの復帰はチェーンの中間地点であり、完了シグナルではない。
...
> **実験的機能**: `isolation: worktree` を使用した並列処理。ドキュメントが不完全なため（[claude-code#27023](https://github.com/anthropics/claude-code/issues/27023)）、上級ユーザー向け。

完全に独立したファイルセットを操作する Issue 群を、worktree 分離で並列処理する。各 Issue が独立した PR を作成する。

**起動**: `--parallel` フラグ付きで複数 Issue を指定（例: `/working-on-issue #101 #102 #103 --parallel`）。`--parallel=N` で同時起動数を指定可（デフォルト 3、最大 5）。

詳細は [reference/batch-workflow.md](reference/batch-workflow.md) の「並列バッチモード」セクションを参照。
...
| 複数 Issue + `--parallel` | `#101 #102 #103 --parallel` | 並列バッチモード（実験的） |
...
| `--parallel` | 並列バッチモード（実験的）。worktree 分離で Issue を並列処理する |
| `--parallel=N` | 並列バッチモードの同時起動数を指定（デフォルト 3、最大 5） |

### フラグの組み合わせ

| 組み合わせ | 動作 |
|-----------|------|
| `--headless --parallel` | ヘッドレス UCP 付き並列バッチモード。トークンコスト警告（AskUserQuestion）はスキップされ、確認なしで実行する |
| `--headless`（単一 Issue） | 単一 Issue のヘッドレスモード（ヘッドレスモードセクション参照） |
| `--parallel`（単一 Issue） | 無視。単一 Issue は通常モードで処理 |
...
- **チェーン自律進行（最重要）**: `action: CONTINUE` を受け取ったら、テキスト出力ではなくツール呼び出しで応答する。TaskList に pending ステップがある限り、同じレスポンス内で次の Agent/Bash ツールを呼び出す。特に `open-pr-issue` 完了後は断絶しやすいため、Work Summary → Status Update を Bash で即座に実行する
