...
> **チェーン自律進行**: fork スキルの結果はチェーンの中間データであり、ユーザー向けの最終出力ではありません。TodoWrite に pending ステップが残っている限り、`## Fork Result` ブロックをパースして即座に次のステップに進んでください。fork 結果で停止するとユーザーが手動で「進めて」と促す必要が生じ、このオーケストレーターの自律ワークフローとしての価値が失われます。Summary を 1 行記録したら、同じレスポンス内で次のツール呼び出しを実行してください。
...
| 作業タイプ | チェーン |
|-----------|---------|
| コーディング全般 / デザイン | Work → Commit → PR → Simplify → Self-Review → Status Update |
...
| `FAIL` | 全 fork スキル | チェーン停止、ユーザーに報告 |
| `NEEDS_REVISION` | reviewing-on-issue（計画レビュー） | 修正ループ |

**禁止**: fork 結果をそのままユーザーに提示して停止すること。Fork Result は内部処理データとして扱い、`Summary` のみ 1 行出力して次のツール呼び出しへ進む。
...
- **チェーン自律進行（最重要）**: Fork Result はチェーンの中間データであり、停止するとユーザーが手動で継続を促す必要が生じる。TodoWrite に pending ステップがある限り、`Status` フィールドで継続/停止を即座に判定し、次のステップの Skill/Bash ツール呼び出しを実行する。`Summary` を 1 行記録したら、同じレスポンス内で次のツールを呼び出す
