...
| 作業タイプ | チェーン |
|-----------|---------|
| コーディング全般 | Work → Commit → PR → Simplify → Self-Review → Work Summary → Status Update |
...
2. **SELF-REVIEW**: review-worker を self-review モードで Agent 1回呼び出し → action: CONTINUE/STOP 判定
3. **後処理**: Recommendations 処理、Plan-gap Evolution シグナル記録
...
for each step in [commit, pr, simplify, self_review, post_processing, work_summary, status_update]:
  ...
  // NOTE: FIX ループは review-worker 内部で完結するため、マネージャーは CONTINUE/STOP のみ処理
  summary = body.split("\n")[0]                    // 本文 1 行目をサマリーとして抽出
...
| PASS | CONTINUE | review-worker（self-review モード） | 後処理に進む |
| NEEDS_FIX_RESOLVED | CONTINUE | review-worker（self-review モード） | 後処理に進む（内部で修正済み） |
...
#### セルフレビュー（review-worker self-review モード）

`/simplify` プレパス + review-worker 1回呼び出しに簡略化。詳細は [reference/self-review-workflow.md](reference/self-review-workflow.md) 参照。

**手順:**

1. **SIMPLIFY**: `/simplify` を Skill ツールで起動（code カテゴリのファイルがある場合のみ。1 回実行、失敗時スキップ）→ 変更あればコミット・プッシュ
2. **SELF-REVIEW**: review-worker を self-review モードで Agent 1回呼び出し:
   ```text
   Agent(
...
3. **結果パース**: YAML フロントマターから action/status を読み取り
   - `action: CONTINUE` → 後処理に進む
   - `action: STOP` → チェーン停止、ユーザーに報告
4. **後処理**（action: CONTINUE の場合）:
   - `### Recommendations` の `[trivial]` 項目 → その場で対応を提案（AskUserQuestion）
   - `### Recommendations` の `[rule]` 項目 → Evolution シグナルとして記録
   - `### Recommendations` の `[trigger:*]` / `[one-off]` 項目 → フォローアップ Issue 作成を提案
   - Plan-gap カウント > 0 → Evolution シグナルとして `planning-on-issue` の改善材料に記録

> **⚠ SIMPLIFY ≠ セルフレビュー**: SIMPLIFY は品質ベースライン向上の前処理。SIMPLIFY 完了後、必ず review-worker の self-review モードを起動すること。
...
チェーン正常完了後（チェーン失敗時はスキップ）、`rule-evolution` ルールの「スキル完了時の自動記録手順」に従い、セッション中に発生した Evolution シグナルを自動記録する。環境チェック（lint メトリクス）を含む（`working-on-issue` は `rule-evolution` ルールの環境チェック対象スキル）。
...
- セルフレビューは review-worker の self-review モードに委任（[reference/self-review-workflow.md](reference/self-review-workflow.md) 参照）
