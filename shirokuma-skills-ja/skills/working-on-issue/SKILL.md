...
> **チェーン自律進行（最重要ルール）**: サブエージェント（Agent ツール）が結果を返したら、**同じレスポンス内で必ず次のツールを呼び出す**。これが唯一かつ最重要のルールである。TaskList に pending ステップが残っているのにテキストのみで応答を終えることはチェーン断絶エラーであり、ユーザーが「続けて」と手動で促す羽目になる。`action: CONTINUE` → 即座に次の Agent/Bash ツール呼び出し。`action: STOP` → チェーン停止しユーザーに報告。
...
|-----------|---------|------------|---------|
| コーディング全般 | 実装、修正、リファクタ、設定、Markdown 編集 | `code-issue` (Skill) | はい（実装・修正・リファクタ） |
| 調査 | キーワード: `research`, `調査` | `researching-best-practices` (subagent) | いいえ |
| レビュー | キーワード: `review`, `レビュー` | `review-issue` (Skill) | いいえ |
...
**出力パースチェックポイント** — サブエージェント出力を受け取ったら:
...
**核心: `action: CONTINUE` を受け取ったら、テキスト出力ではなくツール呼び出しで応答する。**

例外: `ucp_required: true` または `suggestions_count > 0` の場合、AskUserQuestion でユーザーに提示してから続行（[reference/worker-completion-pattern.md](reference/worker-completion-pattern.md) 参照）。

**Tasks 継続不変条件**: サブエージェントスキル完了後、TaskList を確認する。`pending` ステップが残っている場合、同じレスポンス内で次のツール呼び出しを**必ず**実行すること — pending ステップが残ったままテキストのみの最終レスポンスを生成するのはチェーン断絶エラーである。
...
**`open-pr-issue` 完了後のマネージャー管理ステップ（断絶最多ポイント）:**

`open-pr-issue` は `next` フィールドを持たない（次はサブエージェントではなくマネージャーの直接実行）。PR が作成された時点でチェーンが終わったように見えるが、**TaskList には pending ステップが残っている**。`open-pr-issue` の結果を受け取ったら、停止せずに**同じレスポンス内で**以下を Bash ツールで順次実行する:

1. **Work Summary**: Issue コメントとして作業サマリーを投稿（Bash: `shirokuma-docs issues comment`）
2. **Status Update**: `shirokuma-docs issues update {number} --field-status "Review"`（Bash）
3. **Evolution**: シグナル自動記録（ステップ 6 参照）

> **なぜここで断絶するのか**: PR 作成は視覚的な「完了感」が強く、LLM がサマリーを出力して停止しやすい。しかし TaskList の pending ステップが 0 になるまではチェーン途中である。

**構造化データによるチェーン進行ロジック（擬似コード）:**

```text
for each step in [commit, pr, work_summary, status_update]:
  // GUARD: TaskList に pending ステップあり → この反復を必ず実行する（停止禁止）
  ...
  if TaskList.any(status == "pending"):          // pending 残あり → 必ず継続
    invoke_skill(frontmatter.next)                // 同じレスポンス内で次のスキルを起動
  // 全タスクが completed の場合のみチェーン終了
```
...
スキルは Skill ツール（メインコンテキスト）または Agent ツール（サブエージェント）で起動する。プロジェクト固有ルール（paths ベース）が必要なスキルはメインコンテキストで Skill ツール経由で実行する。単純な操作（git, gh）はサブエージェントで実行する。

| スキル | 起動方法 | 理由 |
|--------|---------|------|
| `code-issue` | Skill ツール | プロジェクトルール（development-patterns, security-boundary 等）が必要 |
| `review-issue` | Skill ツール | ルール準拠レビューにプロジェクトルールが必要 |
| `reviewing-claude-config` | Skill ツール | 品質基準にプロジェクトルールが必要 |
| `commit-issue` | Agent (`commit-worker`) | git 操作のみ、プロジェクトルール不要 |
| `open-pr-issue` | Agent (`pr-worker`) | GitHub 操作のみ、プロジェクトルール不要 |
| `researching-best-practices` | Agent (`research-worker`) | 外部調査、プロジェクトルール不要 |
...
> **重要 — Agent ツール復帰後のチェーン継続**: サブエージェント（例: `pr-worker`, `commit-worker`）が完了して Agent ツールが復帰した時点で、**TaskList の残り `pending` ステップを確認する**。pending ステップが残っている場合（作業サマリー、ステータス更新、Evolution）、**同じレスポンス内で即座に次の pending ステップを実行すること**。停止・サマリー表示・ユーザーへの確認は禁止。Agent ツールの復帰はチェーンの中間地点であり、完了シグナルではない。
...
> **注意:** Spec Review / Ready 以外のステータス（In Progress, Preparing, Backlog 等）の Issue に `--headless` を指定した場合も前提条件エラーで停止する。Preparing ステータスの Issue は `preparing-on-issue` による対話的な計画策定が必要なため、ヘッドレスモードの対象外。
...
- このスキルは作業の**マネージャー（メインプロセスの AI エージェント）**であり、Skill ツール（code-issue, review-issue, reviewing-claude-config）または Agent ツール（commit-worker, pr-worker, research-worker）経由で作業を委任する
...
- **チェーン自律進行（最重要）**: `action: CONTINUE` を受け取ったら、テキスト出力ではなくツール呼び出しで応答する。TaskList に pending ステップがある限り、同じレスポンス内で次の Skill/Agent/Bash ツールを呼び出す。特に `open-pr-issue` 完了後は断絶しやすいため、Work Summary → Status Update を Bash で即座に実行する
