...
| `creating-pr-on-issue` | — | **マネージャー管理ステップ開始**（下記参照） | fork に委任しない |
...
2. **REVIEW**: `reviewing-on-issue` / `reviewing-claude-config` を Skill ツールで起動 → PASS/NEEDS_FIX/FAIL 判定
...
> **⚠ SIMPLIFY ≠ セルフレビュー**: SIMPLIFY は品質ベースライン向上の前処理であり、セルフレビューの代替ではない。SIMPLIFY 完了後（変更あり・なし・失敗いずれの場合も）、必ず [REVIEW] に進み `reviewing-on-issue` / `reviewing-claude-config` を Skill ツールで起動すること。SIMPLIFY のみで [REVIEW] をスキップすることは禁止。

| 状態 | アクション |
|------|----------|
| SIMPLIFY | `/simplify` を Skill ツールで起動（code カテゴリのファイルがある場合のみ。1 回実行、失敗時スキップ）**← 前処理・スキップ可** |
| REVIEW | `reviewing-on-issue` / `reviewing-claude-config` を fork で起動 **← スキップ不可。SIMPLIFY 完了後は必ず実行すること。** |
...
2. **サブ Issue の一括作成**: 計画の `### サブ Issue 構成` テーブルを解析し、各行についてサブ Issue を CLI で作成:
   ```bash
   shirokuma-docs issues create --title "{タイトル}" --issue-type "{type}" \
