...
- **チェーン自律進行（最重要）**: fork スキルが Fork Result を返した後、**絶対にユーザーの入力を待たない**。`Status` フィールドで継続/停止を即座に判定する。TodoWrite に pending ステップがある限り、即座に次のステップの Skill/Bash ツール呼び出しを実行する。Fork Result は「ユーザーへの提示物」ではなく「内部処理データ」として扱う。`Summary` を 1 行記録したら、同じレスポンス内で次のツールを呼び出す
