...
| 4 | セルフレビューを実行し修正する | セルフレビュー実行中 | マネージャーが直接管理（リファレンス参照） |
...
**事前解決ロジック**: fork ワーカーは `AskUserQuestion` を使用できないため、マネージャーが起動前にエッジケースを解決する:

| エッジケース | マネージャーの事前アクション |
|------------|--------------------------|
...
**チェーン完了保証**: 各 fork スキルの結果が返却された後、マネージャーは**次のステップに即座に進む**。fork 結果のサマリー表示は 1 行で完了し、ユーザーの入力を待たない。チェーン末尾の Status Update は fork ではなくマネージャーが直接実行するため、分断リスクがない。

#### セルフレビューループ（マネージャーが直接管理）

PR 作成後、マネージャーがセルフレビューを直接管理する。詳細は [reference/self-review-workflow.md](reference/self-review-workflow.md) 参照。
...
| ツール | タイミング |
|--------|-----------|
| AskUserQuestion | 要件確認、アプローチ選択、エッジケース判断（マネージャーが事前解決） |
| TodoWrite | チェーンステップ登録（全作業で必須） |
| Bash | Git 操作、`shirokuma-docs issues` コマンド |

## 注意事項

- このスキルは作業の**マネージャー**（実作業は fork ワーカーに委任）
...
- セルフレビューはマネージャーが直接管理（[reference/self-review-workflow.md](reference/self-review-workflow.md) 参照）
