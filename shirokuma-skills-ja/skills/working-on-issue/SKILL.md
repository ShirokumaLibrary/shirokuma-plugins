...
|---|---------|------------|--------|
| 1 | 実装する | 実装中 | `coding-on-issue` (subagent) / `designing-ui-on-issue` |
| 2 | 変更をコミット・プッシュする | コミット・プッシュ中 | `committing-on-issue` (subagent) |
| 3 | プルリクエストを作成する | プルリクエストを作成中 | `creating-pr-on-issue` (subagent) |
| 4 | セルフレビューを実行し修正する | セルフレビュー実行中 | マネージャー直接管理: SIMPLIFY → REVIEW → COMPLETE |
| 5 | 作業サマリーを投稿する | 作業サマリーを投稿中 | マネージャー直接: `issues comment` |
| 6 | Status を Review に更新する | Status を Review に更新中 | マネージャー直接: `issues update` |
...
|-----------|---------|------------|---------|
| コーディング全般 | 実装、修正、リファクタ、設定、Markdown 編集 | `coding-on-issue` (subagent) | はい（実装・修正・リファクタ） |
...
| `creating-pr-on-issue` | — | **マネージャー管理ステップ開始**（下記参照） | サブエージェントに委任しない |

**`creating-pr-on-issue` 完了後のマネージャー管理ステップ（必須チェックリスト）:**

`creating-pr-on-issue` が結果を返した時点で、以下のステップを**同じチェーン内で**順次実行する。各ステップは TodoWrite に `pending` として登録済みであり、pending が残っている限り停止してはならない:

1. **SIMPLIFY**: `/simplify` を Skill ツールで起動（code カテゴリのみ、失敗時スキップ）→ 変更あればコミット・プッシュ
2. **REVIEW**: `reviewing-on-issue` / `reviewing-claude-config` を Agent ツール（サブエージェント）で起動 → PASS/NEEDS_FIX/FAIL 判定
3. **COMPLETE**: out-of-scope Issue 作成 → 対応完了コメント投稿
4. **Work Summary**: Issue コメントとして作業サマリーを投稿
5. **Status Update**: `shirokuma-docs issues update {number} --field-status "Review"`
6. **Evolution**: シグナル自動記録（ステップ 6 参照）
...
> **重要 — Agent ツール復帰後のチェーン継続**: カスタムサブエージェント（例: `pr-worker`, `commit-worker`）が完了して Agent ツールが復帰した時点で、**TodoWrite の残り `pending` ステップを確認する**。pending ステップが残っている場合（セルフレビュー、作業サマリー、ステータス更新、Evolution）、**同じレスポンス内で即座に次の pending ステップを実行すること**。停止・サマリー表示・ユーザーへの確認は禁止。Agent ツールの復帰はチェーンの中間地点であり、完了シグナルではない。
...
セルフレビューは Agent ツール（サブエージェント）で `reviewing-on-issue` / `reviewing-claude-config` を起動する。レビュースキルは PR コメントの投稿をワークフローに含むため、他の手段で代替するとレビュー結果が PR に記録されず、監査証跡が失われる。

**状態遷移概要:**

```text
[SIMPLIFY] /simplify 初期パス → コミット・プッシュ（変更あり時）【前処理、スキップ可】
    ↓ ※ SIMPLIFY の結果（変更あり/なし/失敗）に関わらず、必ず次の [REVIEW] に進む
[REVIEW] レビュー起動 → [PARSE] 結果パース → [PRESENT] 結果提示 → 判定【スキップ不可】
  ├── PASS → [COMPLETE]
  ├── NEEDS_FIX → [FIX] Task 修正 → [CONVERGE] 収束判定 → [REVIEW]
  └── FAIL → チェーン停止、[REPORT]
```

> **⚠ SIMPLIFY ≠ セルフレビュー**: SIMPLIFY は品質ベースライン向上の前処理であり、セルフレビューの代替ではない。SIMPLIFY 完了後（変更あり・なし・失敗いずれの場合も）、必ず [REVIEW] に進み `reviewing-on-issue` / `reviewing-claude-config` を Agent ツール（サブエージェント）で起動すること。SIMPLIFY のみで [REVIEW] をスキップすることは禁止。

| 状態 | アクション |
|------|----------|
| SIMPLIFY | `/simplify` を Skill ツールで起動（code カテゴリのファイルがある場合のみ。1 回実行、失敗時スキップ）**← 前処理・スキップ可** |
| REVIEW | `reviewing-on-issue` / `reviewing-claude-config` をサブエージェントで起動 **← スキップ不可。SIMPLIFY 完了後は必ず実行すること。** |
| PARSE | 構造化データパース、PASS/NEEDS_FIX/FAIL 判定（`### Detail` から修正方針を決定） |
| PRESENT | セルフレビュー結果サマリーをユーザーに提示 |
| FIX | `Task(general-purpose)` に修正を委任 |
| CONVERGE | 収束判定（数値ベース、2 回連続非減少で停止）|
| REPORT | 残存問題をユーザーに報告 |
| COMPLETE | out-of-scope Issue 作成 → 対応完了コメント投稿 → **後続ステップ**: Work Summary 投稿 → Status → Review 更新 → Evolution シグナル記録 |

**COMPLETE 後の後続ステップは TodoWrite の pending として管理される。COMPLETE 到達後も pending ステップが残っている限り、即座に次のステップを実行すること。**
...
エピックに `## 計画` と `### サブ Issue 構成` セクションが必要。計画がなければ `planning-on-issue` に先に委任（通常フロー）。
...
3. **実行順序の提案**: `### 実行順序` セクションまたは依存列に基づき、AskUserQuestion で推奨順序を提示:
   ```
   サブ Issue を作成しました。依存関係に基づく推奨実行順序:
   1. #{sub1} - {タイトル}（依存なし）
   2. #{sub2} - {タイトル}（#{sub1} に依存）
   #{sub1} から開始しますか？
   ```

4. **最初のサブ Issue の作業開始**: ユーザー確認後、エピック → In Progress に設定し、`working-on-issue #{first-sub-issue}` を再帰的に起動。サブ Issue フローは `parentIssue` フィールドで integration ブランチを自動検出する。
