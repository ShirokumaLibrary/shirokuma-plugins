...
| PARSE | Fork Result パース、PASS/NEEDS_FIX/FAIL 判定（`### Detail` から修正方針を決定） |
...
- **チェーン自律進行**: Fork Result はチェーンの中間データであり、停止するとユーザーが「続けて」と手動で促す必要が生じ、自動ワークフローチェーンの意義を失う。TodoWrite に pending ステップがある限り、`Status` フィールドで継続/停止を即座に判定し、次のステップの Skill/Bash ツール呼び出しを実行する。`Summary` を 1 行記録したら、同じレスポンス内で次のツールを呼び出す
