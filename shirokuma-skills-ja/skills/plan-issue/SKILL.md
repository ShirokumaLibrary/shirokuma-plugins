...
計画作成完了のため、`--via` オプションで Backlog → In progress → Review を 1 コマンドで実行する（直遷移は定義されていないが `--via` が中間ステータスを経由して 2 段階遷移する、#2244）:

```bash
shirokuma-docs status transition {PLAN_ISSUE_NUMBER} --to "Review" --via "In progress"
```

> **後方互換**: `--via` 未対応の旧 CLI を使用している場合は、従来どおり 2 コマンドで遷移する:
>
> ```bash
...
- 計画 Issue は `Backlog` で作成し、作成後に `Backlog → In Progress → Review` の 2 ステップで遷移する。ラベルは `area:plan`
