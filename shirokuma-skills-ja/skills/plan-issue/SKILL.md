...
Issue 本文へ計画のサマリーリンクセクションを書き込む。これにより `review-issue` が `shirokuma-docs items pull {number}` でキャッシュした `.shirokuma/github/{number}.md` から計画リンクを取得してコメントの詳細を参照できる。
...
**重要**: 既存の本文（概要、タスク、成果物等）は保持し、`## 計画` セクションを**追加**する。`shirokuma-docs items pull {number}` でキャッシュした `.shirokuma/github/{number}.md` の内容を既存本文のベースとして使用する場合、`---` で囲まれた YAML フロントマターブロック（メタデータ）を必ず除去してから書き込むこと。
