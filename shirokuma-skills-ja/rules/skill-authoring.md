...
| 配置パターン | 条件 | 例 |
|-------------|------|-----|
| `## 完了レポート` 独立セクション | ワークフローのステップ数が 5 以上 | managing-rules, evolving-rules |
| ステップ内サブセクション | ワークフローのステップ数が 4 以下 | committing-on-issue |

テンプレートは ` ```markdown ` フェンスドコードブロックで定義する。

### コードブロック言語タグ

| コンテンツ | 言語タグ |
|-----------|---------|
| 完了レポートテンプレート | `markdown` |
| CLI コマンド例 | `bash` |
| 設定ファイル例 | `yaml` / `json` |
