...
以下のケースでは、上記の自動進行ルールに対する**例外**として、サブエージェント結果後にユーザー確認（`AskUserQuestion`）を挟む:

| スキル | UCP の位置 | 理由 |
|--------|-----------|------|
| `reviewing-on-pr` | `review-worker` 完了後（スレッド対応開始前） | 修正方針はユーザーが事前確認すべき意思決定 |

UCP はチェーンの「一時停止」であり、チェーンの「中断」ではない。ユーザーが承認すれば自動進行を再開する。
