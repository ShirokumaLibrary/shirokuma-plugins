...
| PR 作成完了 | → Review | `create-pr-issue` | `issues update {n} --field-status "Review"` |
...
3. **イベント駆動**: Status 変更はイベント発生時に即座に実行する（`create-pr-issue` が PR 作成後に Review、`pr merge` が Done）
