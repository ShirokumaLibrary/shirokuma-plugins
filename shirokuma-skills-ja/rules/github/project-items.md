...
| トリガー | アクション | 責任者 | コマンド |
|---------|----------|--------|---------|
| 計画策定開始 | → Preparing + アサイン | `prepare-flow` | `items pull {n}` → frontmatter 編集 → `items push {n}` + `--add-assignee @me` |
...
> `review-issue` は `shirokuma-docs show {number}` で取得した本文のリンクからコメントの詳細計画を参照する。
...
### `show` vs サブコマンド直接呼び出しの使い分け

| コマンド | 返却内容 | 用途 |
|---------|---------|------|
| `shirokuma-docs show {number}` | 本文 + コメント全件 | Issue/PR/Discussion の内容確認、レビュー、実装前調査 |
| `shirokuma-docs issues show {number}` | 本文のみ | フィールド値（Status/Priority 等）の確認のみ |
...
AI が Issue/PR/Discussion の内容を確認する場合は、**`shirokuma-docs show {number}` を使いコメント全件を取得する**。これにより、以下の情報を1回のコマンドで把握できる:
