...
graph LR
  ...
  Closed[Closed/Done]
  DoneOpen -->|親 Close で連動 Close| Closed
```
...
| Done(Open) | 子 Issue の作業完了。GitHub Issue は Open のまま。親 Issue Close 時に連動 Close される |
| Done / Closed | 完了・クローズ済み |
| Cancelled | 明示的に見送り（`issue cancel` で設定） |

> **@deprecated**: `Ready` および `Completed` は既存 Issue との互換のため残るが、新規遷移先としては使用しない。脱出パスのみ維持（`Ready → In Progress`、`Completed → Done`）。

### status approve と Done(Open) 状態

`status approve {number}` は Review 状態の Issue を Done(Open) に遷移させる専用 CLI（ADR-v3-013）。

- **遷移**: Review → Done（GitHub Issue state は Open のまま）
- **Review 以外では失敗**: `result: "error"` で exit 1
- **JSON 出力**: `{ "result": "ok" | "error", "message": "...", "next_suggestions": [...] }`

Done(Open) は「子 Issue の作業完了だが Open 維持」の中間状態。親 Issue が Close されると `syncChildCloseOnParentClose` により自動的に Close される。
...
| Status | 説明 | 遷移トリガー |
|--------|------|-------------|
| Review | PR 作成直後 | `pr create` が PR を Projects に追加時に自動設定 |
| Done | マージ後 | `pr merge` で自動設定 |

**使用しないステータス**: Backlog, Pending, Ready, In Progress, On Hold, Completed, Cancelled は PR には適用しない。

> `integrity` は PR ステータスの不整合を検出する（OPEN PR が Done、MERGED/CLOSED PR がアクティブステータス、Issue 専用ステータスの使用）。
...
|----------------|----------------|
| 全サブ Issue が Done(Open) または Done | 親を Done に自動遷移 |
| 一部が In Progress / Review | 親を In Progress に維持 |
| 一部が Done(Open) + 残りが Backlog | 親を In Progress に維持（進行中とみなす） |
| 全サブ Issue が Cancelled | 親を Backlog に自動戻し |

**親 Close 時の連動 Close**: 親 Issue が Close されると、`Done(Open)` 状態の子 Issue は `syncChildCloseOnParentClose` により自動的に Close される。

**リアクティブ自動導出**: CLI が `status transition`、`issue close`（`issue cancel` 含む）、`status update-batch`、`pr merge` 実行時にサブ Issue のステータス変更を検出し、親のステータスを自動的に導出・更新する。明示的な `integrity --fix` の実行は不要（バッチ整合性チェックとしては引き続き利用可能）。

### 計画リセットフロー

エピックの計画を白紙に戻す場合（サブ Issue が作成済みの場合）:

1. 全サブ Issue を `issue cancel {sub-numbers}` で Cancelled に変更（CLI が自動的に親を Backlog に遷移）
2. `prepare-flow` で再計画
...
| トリガー | アクション | 責任者 | コマンド |
|---------|----------|--------|---------|
| 計画策定開始 | → In Progress + アサイン | `prepare-flow` | `status transition {n} --to "In progress"` |
| 計画策定完了 | → Review | `prepare-flow` | `status transition {n} --to Review` |
| 設計開始 | → In Progress | `design-flow` | `status transition {n} --to "In progress"` |
| 設計完了 | → Review | `design-flow` | `status transition {n} --to Review` |
| ユーザーが計画承認、実装開始 | → In Progress + ブランチ | `implement-flow` | `status transition {n} --to "In progress"` |
| implement-flow 全工程完了 | → Review | `implement-flow` | `status transition {n} --to Review`（PR 作成・simplify・security-review・作業サマリー後） |
| ユーザーが明示承認 | → Done(Open) | `approve` スキル / 手動 | `status approve {n}` |
| review-flow 開始時 | → In Progress | `review-flow` | `status transition {n} --to "In progress"` |
| review-flow レビュー対応完了後 | → Review | `review-flow` | `status transition {n} --to Review` |
| マージ | → Done | `commit-issue` (via `pr merge`) | 自動更新 |
| ブロック | → On Hold | 手動 | `status transition {n} --to "On Hold"` + 理由コメント |
| 完了（PR不要） | → Done | 手動 | `status update-batch --done {n}` |
| キャンセル | → Cancelled | `issue cancel` | `issue cancel {n}` |
| 計画承認 | → Done（計画 Issue） | `implement-flow` | `status transition {plan-n} --to Done`（エピック時は全サブ Issue Done 確認後） |
...
| Issue リオープン | `issue reopen` が Status 復元 | `Item reopened` が Status → Backlog | 競合の可能性あり |

**原則:**
- CLI が**権威ある Status 更新**を行う（タイムスタンプ更新・親 Issue 導出を含む）
- Workflows は**バックストップ**として機能する（CLI を経由しない手動操作をカバー）
- 二重実行は冪等性により実害なし
- リオープン時のみ CLI の Status 復元と Workflows の Backlog 設定が競合し得るが、CLI 実行後に Workflows が上書きする可能性がある。競合した場合は `shirokuma-docs status update-batch {number} --status {正しいステータス}` で修正する

## Issue 作成時の初期ステータス制約

`issue add` コマンドで Issue を作成する際、`status` フィールドに指定できる初期ステータスは以下のみ:

| ステータス | 許可 | 用途 |
|-----------|------|------|
| `Backlog` | はい | 通常の新規 Issue（デフォルト） |
| `Pending` | はい | トリアージ未対応の Issue |
| `Review` 以降 | いいえ | 作成後に `status transition` で遷移する |
...
| コマンド | 返却内容 | 用途 |
|---------|---------|------|
| `shirokuma-docs issue context {number}` | 本文 + コメント全件（キャッシュ） | Issue/PR/Discussion の内容確認、レビュー、実装前調査 |
| `shirokuma-docs issue show {number}` | 本文のみ | フィールド値（Status/Priority 等）の確認のみ |
| `shirokuma-docs pr show {number}` | 本文のみ | PR メタデータ（ブランチ、変更数等）の確認のみ |
| `shirokuma-docs discussion show {number}` | 本文のみ | Discussion 本文の確認のみ |

### コメント全件読み込みを前提とするワークフロー

AI が Issue/PR/Discussion の内容を確認する場合は、**`shirokuma-docs issue context {number}` を使いコメントをキャッシュし、`.shirokuma/github/{org}/{repo}/issues/{number}/body.md` を Read ツールで読み込む**。これにより、以下の情報を把握できる:
