...
**使用しないステータス**: Backlog, Preparing, Designing, Ready, Icebox, In Progress, Testing, Released, Pending, Not Planned は PR には適用しない。
...
| 全サブ Issue が Completed または Done/Released | 親を Review に自動遷移 |
...
| review-flow 開始時 | → In Progress | `review-flow` | frontmatter `status: "In Progress"` → `items push {n}` |
| review-flow レビュー対応完了後 | → Review | `review-flow` | frontmatter `status: "Review"` → `items push {n}` |
...
| 計画承認 | → Done（計画 Issue） | `implement-flow` | `subIssuesSummary` から計画 Issue を特定 → `items pull {plan-n}` → frontmatter `status: "Done"` → `items push {plan-n}`（エピック時は全サブ Issue Done 確認後） |
