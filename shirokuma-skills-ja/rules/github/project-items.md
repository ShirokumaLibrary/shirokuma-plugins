...
### items approve と Done(Open) 状態

`items approve {number}` は Review 状態の Issue を Done(Open) に遷移させる専用 CLI（ADR-v3-013）。
...
| 設計開始 | → In Progress | `design-flow` | `items transition {n} --to "In Progress"` |
| 設計完了 | → Review | `design-flow` | `items transition {n} --to Review` |
...
| ユーザーが明示承認 | → Done(Open) | `approve` スキル / 手動 | `items approve {n}` |
