...
**使用しないステータス**: Backlog, Preparing, Designing, Spec Review, Ready, Icebox, Testing, Released, Pending, Not Planned は PR には適用しない。

> **注意**: `items integrity` による PR ステータスの不整合検出は未実装。改修は別 Issue で対応予定。
...
| Status | 説明 | 遷移トリガー |
|--------|------|-------------|
| Spec Review | 計画策定完了、レビュー待ち | `prepare-flow` が計画作成後に設定 |
| Done | 計画承認済み | `implement-flow` が実装開始時、または手動承認時 |

**`items integrity` の集計除外**: 親 Issue のステータス自動導出時、`area:plan` ラベルの計画 Issue はサブ Issue ステータス集計から除外する。これにより、計画 Issue が Spec Review のまま残っていても親の In Progress 導出に影響しない。

> **注意**: `classifyParentStatusInconsistencies` の `area:plan` 除外ロジックは未実装。改修は別 Issue で対応予定。
...
# → .shirokuma/github/{plan-issue-number}.md を Read ツールで読み込む
```
