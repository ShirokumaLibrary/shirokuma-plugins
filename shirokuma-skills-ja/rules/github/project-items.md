...
| PR 作成完了 | → Review | `creating-pr-on-issue` | `issues update {n} --field-status "Review"` |
| マージ | → Done | `committing-on-issue` (via `pr merge`) | 自動更新 |
...
`preparing-on-issue` オーケストレーターが計画を開始するとき、Backlog → Preparing に遷移する。

- **目的**: 計画策定中であることの可視化、計画開始タイムスタンプの記録
- **入口**: `preparing-on-issue` が `planning-worker` に委任する前に設定
- **出口**: 計画完了後 → Designing（設計が必要な場合）または Spec Review（`preparing-on-issue` が計画レビュー通過後に設定）
- **pre-work ステータス**: `WORK_STARTED_STATUSES` に含まれない（Spec Review と同様）

### Designing の運用

`designing-on-issue` オーケストレーターが設計作業を担当する（Preparing と Spec Review の間）。

- **目的**: 設計作業中であることの可視化
- **入口**: `preparing-on-issue` が設計フェーズ必要と判断時に設定（Step 5a）
- **出口**: 設計完了 → Spec Review
- **pre-work ステータス**: `WORK_STARTED_STATUSES` に含まれない（Preparing と同様）

### Spec Review の運用

`preparing-on-issue` オーケストレーターが `planning-worker` の計画完了と計画レビュー通過を受けて、Preparing → Spec Review に移行する。

- **目的**: ユーザーが計画を確認・承認するゲート
- **入口**: `preparing-on-issue` が計画レビュー通過後に設定（`planning-on-issue` が `planning-worker` 経由で計画を書き込み）
...
3. **イベント駆動**: Status 変更はイベント発生時に即座に実行する（`creating-pr-on-issue` が PR 作成後に Review、`pr merge` が Done）
