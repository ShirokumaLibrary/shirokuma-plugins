...
|---------|-------|--------|------|
| Handovers（非推奨） | 🤝 | Open-ended discussion | **非推奨。** 代わりに Issue コメントを使用。下記参照 |
...
> **Handovers 非推奨化**: セッションコンテキストは `ending-session` と `working-on-issue` が Issue コメントとして保存する。Handovers Discussion には保存しない。`starting-session #N` は Issue コメントからコンテキストを復元する。既存の Handovers は参照用に保持するが、Issue バウンドセッションでは新規作成しない。アンバウンドセッション（トリアージ・管理）では過渡的措置として引き続き Handovers を作成する。
...
| カテゴリ | 形式 |
|---------|------|
| Handovers（非推奨） | `YYYY-MM-DD - {サマリー}`（アンバウンドセッションのみ） |
| ADR | `ADR-{NNN}: {タイトル}` |
| Knowledge | `{トピック名}` |
