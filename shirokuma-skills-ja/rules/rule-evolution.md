...
主要スキル（`working-on-issue`, `preparing-on-issue`, `creating-item`）の完了時に、以下の手順でセッション中に発生した Evolution シグナルを自動記録する。各スキルはこのセクションを参照して自動記録を実行する。
...
コード変更を伴うスキル完了時に、プロジェクトの客観的状態を確認する。`preparing-on-issue` / `creating-item` はコード変更を伴わないため対象外。
