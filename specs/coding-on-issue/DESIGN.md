...
`context: fork` で動作するため、`TodoWrite` / `AskUserQuestion` は使用不可。進捗管理とユーザー対話はマネージャー（`working-on-issue`）が担当する。

## トリガーキーワード

fork スキルのため `description` による直接トリガーは想定しない。`working-on-issue` のディスパッチ条件テーブルに基づき委任される。
